...
sshpass -p "$vm_password" ssh -o StrictHostKeyChecking=no $vm_username@$manager_external_ip "git clone https://github.com/wuttinanhi/1-million-connection-challenge.git && cd 1-million-connection-challenge && docker stack deploy -c docker-compose.yml my-stack"
